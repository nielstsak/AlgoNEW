...
if TYPE_CHECKING:
    from src.config.definitions import AppConfig, WfoSettings, GlobalConfig, StrategyParamsConfig, ExchangeSettings
...
from src.config.definitions import WfoSettings, GlobalConfig, StrategyParamsConfig, ExchangeSettings # MODIFIED IMPORT CASE and added others
from src.backtesting.optimizer.optimization_orchestrator import run_optimization_for_fold
from src.backtesting.optimizer.objective_evaluator import ObjectiveEvaluator
from src.backtesting.optimizer.study_manager import StudyManager
from src.backtesting.optimizer.results_analyzer import ResultsAnalyzer
...
from src.config.loader import load_exchange_config # MODIFIED: Used for fallback
...
class WFOGenerator:
    ...
    def _get_expanding_folds(self,
        ...
        if oos_start_pos == len(df_enriched_data.index) or (oos_start_pos < len(df_enriched_data.index) and df_enriched_data.index[oos_start_pos] > oos_total_end_ts) :
        ...
        for i in range(n_splits):
            ...
            start_pos_is_fold = df_is_total_enriched.index.searchsorted(current_fold_is_start_ts_approx, side='right')

            if start_pos_is_fold == 0 :
                current_fold_is_start_ts_actual = actual_is_total_start_ts_from_df
            elif start_pos_is_fold <= len(df_is_total_enriched.index) :
                 # searchsorted 'right' means if exact match, it's the one AFTER. If not exact, it's the one after where it would be.
                 # So usually, we need start_pos - 1, unless start_pos is 0.
                 # Let's refine: find closest available timestamp <= current_fold_is_start_ts_approx
                if start_pos_is_fold > 0 and df_is_total_enriched.index[start_pos_is_fold-1] <= current_fold_is_start_ts_approx :
                     current_fold_is_start_ts_actual = df_is_total_enriched.index[start_pos_is_fold-1]
                elif start_pos_is_fold < len(df_is_total_enriched.index) and df_is_total_enriched.index[start_pos_is_fold] >= current_fold_is_start_ts_approx : # If approx is before first actual data
                     current_fold_is_start_ts_actual = df_is_total_enriched.index[start_pos_is_fold]
                else: # Fallback or end of array
                    current_fold_is_start_ts_actual = actual_is_total_start_ts_from_df # Fallback to total start
            ...
            if current_is_fold_duration_actual < min_is_duration_from_settings_td and n_splits > 1 : # Don't warn if n_splits=1 and it's just short
                 logger.warning(f"Fold {fold_idx}: Actual IS duration {current_is_fold_duration_actual} ({current_is_fold_duration_actual.days} days) is less than min_is_period_days ({self.wfo_settings.min_is_period_days} days). This fold might be too short.")
...
class WalkForwardOptimizer:
    def __init__(self, app_config: 'AppConfig'):
        ...
        self.strategies_config_dict: Dict[str, Any] = {}
        if hasattr(self.app_config.strategies_config, 'strategies') and \
           isinstance(self.app_config.strategies_config.strategies, dict):
            for name, strategy_params_config_obj in self.app_config.strategies_config.strategies.items():
                if dataclasses.is_dataclass(strategy_params_config_obj): # MODIFIED HERE
                    self.strategies_config_dict[name] = dataclasses.asdict(strategy_params_config_obj) # MODIFIED HERE
                elif isinstance(strategy_params_config_obj, dict):
                    self.strategies_config_dict[name] = strategy_params_config_obj
                else:
                    logger.error(f"Strategy config for '{name}' is not a dataclass or dict.")
        else:
            logger.error("AppConfig.strategies_config.strategies is not a dictionary or is missing.")


        self.paths_config: Dict[str, Any] = dataclasses.asdict(self.global_config_obj.paths) # MODIFIED HERE
        ...
        if selected_account_for_client:
            ...
            if credentials and len(credentials) == 2 and credentials[0] and credentials[1]: # Check if key/secret are not None
    ...
    def run(self, pairs: List[str], context_labels: List[str]) -> Dict[str, Any]:
        ...
        for pair_symbol in pairs:
            ...
            if not symbol_info_data:
                ...
                from src.utils.exchange_utils import get_pair_config_for_symbol as get_pair_cfg # Local import
                try:
                    ...
                    if exchange_settings and hasattr(exchange_settings, 'exchange_info_file_path'):
                        # load_exchange_config returns a Dict
                        exchange_info_dict = load_exchange_config(Path(exchange_settings.exchange_info_file_path))
                        symbol_info_data = get_pair_cfg(pair_symbol, exchange_info_dict)
                    if not symbol_info_data:
