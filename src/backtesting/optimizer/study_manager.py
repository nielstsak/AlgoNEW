...
class StudyManager:
    def __init__(self,
        ...
        # Garder une copie des settings originaux chargés depuis le JSON
        self.optuna_settings_original: 'OptunaSettings' = self.app_config.global_config.optuna_settings
        
        # Créer une copie active des settings qui pourra être modifiée par le profil
        ...
        # >>> DEBUT MODIFICATION POUR GESTION DES PROFILS OPTUNA <<<
        ...
        if profile_to_activate and profile_to_activate in available_profiles:
            ...
            # Surcharge des paramètres de base par ceux du profil
            # Utiliser .get() avec une valeur par défaut pour éviter les erreurs si une clé manque dans le profil
            ...
            logger.debug(f"{self.log_prefix} Sampler params after profile: {self.active_optuna_settings.sampler_params}")
            logger.debug(f"{self.log_prefix} Pruner params after profile: {self.active_optuna_settings.pruner_params}")
        else:
            if profile_to_activate:
                logger.warning(f"{self.log_prefix} Optuna profile '{profile_to_activate}' not found in sampler_pruner_profiles. Using default Optuna settings from config_global.json.")
            else:
                logger.info(f"{self.log_prefix} No default_profile_to_activate specified. Using default Optuna settings from config_global.json.")
        # >>> FIN MODIFICATION POUR GESTION DES PROFILS OPTUNA <<<
    ...
    def _create_sampler(self) -> optuna.samplers.BaseSampler:
        """Instantiates the Optuna sampler based on **active** configuration."""
        # Utiliser self.active_optuna_settings qui contient les valeurs du profil (ou les défauts si aucun profil activé)
    ...
    def _create_pruner(self) -> optuna.pruners.BasePruner:
        """Instantiates the Optuna pruner based on **active** configuration."""
        # Utiliser self.active_optuna_settings
    ...
    def run_study(self,
        ...
        # Utiliser self.active_optuna_settings pour les directions et noms d'objectifs
        ...
        try:
            study = optuna.create_study(
                ...
                sampler=self._create_sampler(), # Utilisera les settings actifs
                pruner=self._create_pruner(),   # Utilisera les settings actifs
        ...
        objective_instance = objective_evaluator_class(
            strategy_name=self.strategy_name, # Passer le nom original de la stratégie
        ...
        # Utiliser self.active_optuna_settings pour n_trials et n_jobs
