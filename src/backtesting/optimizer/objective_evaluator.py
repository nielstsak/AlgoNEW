...
try:
    ...
    from src.config.definitions import ParamDetail, AppConfig 
...
class ObjectiveEvaluator:
    def __init__(self,
        ...
        if not self.params_space_details:
            logger.critical(f"CRITICAL: self.params_space_details is EMPTY for strategy {self.strategy_name_key}. This will likely lead to pruning. Raw params_space: {raw_params_space}")
        ...
        logger.debug(f"ObjectiveEvaluator for {self.strategy_class_name} (key: {self.strategy_name_key}) on {self.pair_symbol} initialized. Enriched data shape: {self.df_enriched_slice.shape}")
    ...
    def _calculate_indicator_on_selected_klines(self,
        ...
        if not all_required_cols_present or not ta_inputs.get('close', pd.Series(dtype=float)).notna().any(): # type: ignore
            logger.error(f"Critical source data missing for indicator '{indicator_type}' with prefix '{kline_ohlc_prefix}'.")
            return None
        ...
        try:
            ...
            logger.debug(f"Calculating {indicator_type_lower} with params: {indicator_params} on columns with prefix '{kline_ohlc_prefix}'.")
    ...
    def _prepare_data_with_dynamic_indicators(self, trial_params: Dict[str, Any], trial_number_for_log: Optional[Union[int, str]] = None) -> pd.DataFrame:
        ...
        if atr_period_param is not None and atr_freq_param_raw is not None:
            ...
            if atr_source_col_name in self.df_enriched_slice.columns:
                ...
                if isinstance(df_for_simulation.get('ATR_strat'), pd.Series): logger.info(f"{log_prefix} ATR_strat (1-min dynamically calculated).")
            else:
        ...
        if self.strategy_class_name == "MaCrossoverStrategy":
            ...
            if trial_params.get('anticipate_crossovers', False):
                ...
                if 'MA_SHORT_strat' in df_for_simulation and df_for_simulation['MA_SHORT_strat'].notna().any():
                    try:
                        slope_short_series = ta.slope(df_for_simulation['MA_SHORT_strat'].dropna(), length=slope_period, append=False) # type: ignore
                ...
                if 'MA_MEDIUM_strat' in df_for_simulation and df_for_simulation['MA_MEDIUM_strat'].notna().any():
                    try:
                        slope_medium_series = ta.slope(df_for_simulation['MA_MEDIUM_strat'].dropna(), length=slope_period, append=False) # type: ignore
            ...
            if bb_period is not None and bb_std is not None and bb_freq_raw and bb_freq_raw.lower() != 'none':
                ...
                if bb_df_result is not None and isinstance(bb_df_result, pd.DataFrame) and not bb_df_result.empty and len(bb_df_result.columns) >= 4: # Check for at least 4 columns (BBL, BBM, BBU, BBB)
            ...
            vol_kline_col_strat_expected = f"Kline_{vol_freq_raw}_volume" if vol_freq_raw and vol_freq_raw.lower() not in ["1m", "1min"] else "volume"
        ...
        for param_name, p_detail in self.params_space_details.items():
            ...
            try:
                if p_detail.type == 'int':
                    ...
                    params_for_trial[param_name] = trial.suggest_float(param_name, float(p_detail.low), float(p_detail.high), step=p_detail.step) # type: ignore
                ...
                else: raise optuna.exceptions.TrialPruned(f"Failed to suggest or fallback for param {param_name}")
    ...
    def __call__(self, trial: optuna.Trial) -> Tuple[float, ...]:
        ...
        if self.is_oos_eval:
            ...
            trial_id_for_log = f"OOS_for_IS_Trial_{is_trial_num_oos}" if is_trial_num_oos is not None else f"OOS_Trial_UnknownOrigin_{trial.number if hasattr(trial, 'number') else uuid.uuid4().hex[:6]}"
            trial_number_for_prepare_log = is_trial_num_oos if is_trial_num_oos is not None else (trial.number if hasattr(trial, 'number') else None)
        else:
        ...
        try:
            module_path = self.strategy_script_ref.replace('.py', '').replace('/', '.') # type: ignore
        ...
        try:
            ...
            return tuple([-float('inf') if d == "maximize" else float('inf') for d in self.optuna_objectives_config['objectives_directions']])
        ...
        if not metrics: 
            logger.warning(f"{log_prefix} Simulation returned no metrics. Pruning trial.")
