...
from datetime import datetime # Pour sérialiser datetime dans JSON
...
def main():
    ...
    # Les arguments --config-global et --config-strategies sont omis pour l'instant,
    # car load_all_configs utilise des chemins fixes par rapport à project_root.
    ...
    # Les logs émis avant cet appel utiliseront la configuration basicConfig.
    
    app_config: Optional[AppConfig] = None
    try:
        app_config = load_all_configs(project_root=project_root_arg)
        # À partir d'ici, le logging configuré par l'application est actif.
    ...
    try:
        ...
        try:
            ...
            with open(run_config_path, 'w', encoding='utf-8') as f_cfg:
                # Utiliser dataclasses.asdict pour convertir AppConfig et ses membres en dictionnaires
                # Cela nécessite que toutes les DataClasses imbriquées soient également des dataclasses.
                # EnhancedJSONEncoder s'occupera des types comme Path et datetime.
            ...
            # Continuer l'exécution même si la sauvegarde de la config échoue, mais loguer l'erreur.

        pairs_to_run = [args.pair.upper()]
        contexts_to_run = [args.tf]
        ...
        if wfo_results:
            ...
            # Loguer un résumé des résultats si pertinent
            for combo_key, summary_data in wfo_results.items():
                ...
                logger.info(f"  - Résumé pour '{combo_key}': {num_folds_in_summary} fold(s) traité(s). "
                            f"Voir {wfo_runner.run_output_dir / summary_data.get('strategy_name', '') / summary_data.get('pair_symbol', '') / summary_data.get('context_label', '') / 'wfo_strategy_pair_summary.json'}")
        else:
...
if __name__ == "__main__":
    # Le logger global est configuré dans load_all_configs.
    # Les logs émis avant cet appel (ex: erreurs d'import) utilisent la config de base.
    main()
